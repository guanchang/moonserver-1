...
BufBlock_t* alloc_block(uint32_t length) {
    ...
    if (NULL == block) {
        LOG4CPLUS_INFO(log4cplus::Logger::getRoot(), "receive queue allocate bufblock failed");
        return NULL;
    }
    ...
    if (0 == n) {
        ...
        if (NULL == block->page_base) {
            ...
            LOG4CPLUS_INFO(log4cplus::Logger::getRoot(), "receive queue allocate bufpage failed");
            return NULL;
        }
    }
